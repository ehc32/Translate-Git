...
ubicationEntry   = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\001\entry"
ubicationDestiny = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\001\destiny"
...
def _truncate_lines_for_field(field_tag: str, lines: list[str]) -> list[str]:
    ...
    for ln in lines:
        ...
        if len(s) > 35:
            # We don't have the full XML path here, but we can report the field and value.
            # Format similar to: Field content has been truncated. Value '...' has been altered.
            msg = (
                f"Field {field_tag}: Content truncated/split (exceeded 35 chars).\n"
                f"   Original Value: '{s}'"
            )
            if msg not in TRUNCATION_WARNINGS:
...
if __name__ == "__main__":
    ...
    if TRUNCATION_WARNINGS:
        ...
        report_lines.append(" - NetworkValidation: WARNING.TINPUT: The input message contains potential truncation errors.")
        report_lines.append(" - Translation: WARNING.TOUTUG: Validation of usage guidelines performed locally.")
        
        for w in TRUNCATION_WARNINGS:
            report_lines.append("")
            report_lines.append(w)
            report_lines.append(" - Translation: TRUNC_N.T0000T: Field content has been truncated.")
            
        report_lines.append("---")
        report_lines.append("") # Empty line before message
        exception_report = "\n".join(report_lines)

    # === Final Message Construction ===
    mt_body = finalize_mt_message(build_header_12(ubicationEntry), out_lines)
    full_output = exception_report + mt_body
