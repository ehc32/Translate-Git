...
# -----------------------------------------------------------------------
traslateId = "005"
ubicationEntry = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\004\entry"
ubicationDestiny = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\004\destiny"
...
class FieldContext:
    def __init__(self):
        ...
        # Truncation logging (only when a value is *emitted* to an output field)
        self.truncations: List[Dict[str, Any]] = []
        self._trunc_seen: set = set()
    ...
    def add_truncation(
        ...
        *,
        ...
        try:
            mt_tag = str(mt_tag or "").strip() or "UNKNOWN"
            original_s = "" if original is None else str(original)
            max_len_i = int(max_len or 0)
            if max_len_i <= 0:
                return
            exceeded = len(original_s) - max_len_i
            if exceeded <= 0:
                return

            target_path_s = "" if target_path is None else str(target_path)
            source_path_s = "" if source_path is None else str(source_path)
            ...
            self.truncations.append(
                {
                    ...
                    "exceeded": exceeded,
                    "original": original_s,
                    "target_path": target_path_s,
                    "source_path": source_path_s,
                }
            )
        except Exception:
            # never break translation due to logging
            return


def _infer_first_mt_tag(vs: Any) -> str:
    """Intenta inferir el tag MT (ej. '20', '52A') usado como fuente en un ValueSpec."""
    if isinstance(vs, dict):
        if "mt" in vs:
            return str(vs.get("mt", "")).strip()
        if "mtField" in vs:
            return str(vs.get("mtField", "")).strip()
        for v in vs.values():
            tag = _infer_first_mt_tag(v)
            if tag:
                return tag
    elif isinstance(vs, list):
        for it in vs:
            tag = _infer_first_mt_tag(it)
            if tag:
                return tag
    return ""
...
) -> Tuple[Dict[str, List[str]], FieldContext]:
