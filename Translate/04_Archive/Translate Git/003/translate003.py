...
# -----------------------------------------------------------------------
traslateId = "003"
ubicationEntry = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\003\entry"
ubicationDestiny = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\003\destiny"
...
def _eval_value(
    ...
    # Selecciona el primer valor no vacío de la lista de ValueSpec
    if "first_non_empty" in vs:
        candidates = vs.get("first_non_empty") or []
        for spec in candidates:
            val = _eval_value(dot, ctx, spec)
    ...
    if "substr" in vs:
        ...
        # Logging de truncamiento basado en substr solo cuando
        # estamos generando salida (THEN) y conocemos el campo MT.
    ...
    if "dtfmt" in vs:
        ...
        # Hora con offset HHMM±HHMM (ya existía)
        if out == "HHMM±HHMM":
            return _format_hhmm_offset(iso_val)

        # Otras salidas basadas en fecha/hora ISO (YYMMDD, HHMMSS)
        dt = _parse_iso_dt_norm(iso_val)
        if not dt:
            return ""
        if out == "YYMMDD":
...
def _eval_condition(dot, ctx, cond: CondSpec) -> bool:
    ...
    # Operadores adicionales de texto para reglas 21, etc.
    if op == "starts_with":
        rs = _norm_ws(right)
        res = bool(rs) and ls.startswith(rs)
        dbg(f"COND 'starts_with' left={ls!r} right={rs!r} -> {res}")
        return res
    if op == "ends_with":
        rs = _norm_ws(right)
        res = bool(rs) and ls.endswith(rs)
        dbg(f"COND 'ends_with' left={ls!r} right={rs!r} -> {res}")
        return res
    if op == "contains":
        rs = _norm_ws(right)
        res = bool(rs) and (rs in ls)
...
# -----------------------------------------------------------------------
# Soporte para logging de truncamientos (basado en substr)
# -----------------------------------------------------------------------
...
def write_truncation_log(ts: str) -> None:
    ...
    lines.append("Message Type : MT202")
    lines.append("Exception Report:")
    lines.append("")
    lines.append("Envelope")
    lines.append("FICdtTrf to MT202 Translation Warnings:")
    lines.append(
    ...
    log_name = f"MT202_{ts}_TRUNC.txt"
    log_path = Path(ubicationLog) / log_name
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_bytes("\n".join(lines).encode("utf-8"))


def _collect_then_lines(dot, ctx, then_obj, field_tag: Optional[str] = None):
    lines: list[str] = []
...
if __name__ == "__main__":
    ...
    # Generar log de truncamientos (si hubo) para esta ejecución
