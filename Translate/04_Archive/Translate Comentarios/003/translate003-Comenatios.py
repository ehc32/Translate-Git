...
class FieldContext:
    def __init__(self, truncations: Optional[List[Dict[str, Any]]] = None):
        self.vars: Dict[str, Any] = {}
        # shared list (global) where we append truncation events
        self.truncations: List[Dict[str, Any]] = (
            truncations if truncations is not None else []
        )
        # dedupe key: (mt_tag, max_len, original, target_path)
        self._trunc_seen: set[tuple[str, int, str, str]] = set()

    def add_truncation(
        self, mt_tag: str, original: Any, max_len: int, target_path: str
    ):
        original_str = "" if original is None else str(original)
        max_len_int = int(max_len)
        exceeded = max(0, len(original_str) - max_len_int)
        if exceeded <= 0:
            return
        key = (str(mt_tag), max_len_int, original_str, str(target_path))
...
def _infer_first_xml_suffix(vs: Any) -> str:
    """Intenta inferir el primer path XML (suffix) presente en un ValueSpec.
    Se usa solo para enriquecer el 'path' del Exception Report (fuente del dato).
    """
    try:
        if isinstance(vs, dict):
            if "xml" in vs and vs["xml"]:
                return str(vs["xml"])
            # common wrappers
            for k, v in vs.items():
                if k in ("substr", "pad", "numfmt", "dtfmt", "optional"):
                    # these have nested dicts
                    if isinstance(v, dict):
                        for kk in ("value", "items", "path"):
                            if kk in v:
                                found = _infer_first_xml_suffix(v[kk])
                                if found:
                                    return found
                        # for optional, could contain "value"
                        found = _infer_first_xml_suffix(v)
                        if found:
                            return found
                elif k in (
                    "concat",
            ...
            for it in vs:
                found = _infer_first_xml_suffix(it)
                if found:
                    return found
    except Exception:
        return ""
...
def build_exception_report(truncs: List[Dict[str, Any]]) -> str:
    """Construye el bloque 'Exception Report' (texto) para anteponer a la salida."""
    if not truncs:
        return ""
    lines: List[str] = []
    lines.append("Exception Report:")
    lines.append("Envelope")
    lines.append("PACS.009 to MT202 Translation Warnings:")
    lines.append(
    ...
    for t in truncs:
        ...
        exceeded = int(t.get("exceeded", 0) or 0)
        max_len = int(t.get("max_len", 0) or 0)
        original = t.get("original", "")
        target_path = t.get("target_path", "")
        lines.append(
            f"Field {mt_tag}:content has been truncated due to exceeding the maximum {max_len} allowed length (exceeded {exceeded} chars)."
        )
        lines.append(f"   Original Value: {original!r}")
        lines.append(f"    path {target_path}")
        lines.append(
            " - Translation: TRUNC_N.T0000T: Field content has been truncated."
        )
        lines.append("")
    return "\n".join(lines).rstrip()
...
def _eval_value(
    ...
    if "pad" in vs:
        ...
        inner_str = "" if inner is None else str(inner)
        n = int(conf.get("len", 0) or 0)
        out = _padx(inner_str, n, conf.get("fill", "X"))

        # Truncamiento real SOLO cuando estamos emitiendo el valor hacia un campo destino.
        if ctx.vars.get("_eval_phase") == "emit" and n > 0 and len(inner_str) > n:
            mt_tag = str(ctx.vars.get("_mt_tag") or "")
            tpath = str(ctx.vars.get("_target_path") or "")
            src_xml = _infer_first_xml_suffix(conf.get("value", {}))
            full_path = (
                f"{tpath} <= {src_xml}" if tpath and src_xml else (tpath or src_xml)
            )
            if mt_tag and full_path:
                ctx.add_truncation(
                    mt_tag=mt_tag, original=inner_str, max_len=n, target_path=full_path
                )
    ...
    if "substr" in vs:
        ...
        start = int(conf.get("start", 0) or 0)
        ...
        # Truncamiento real SOLO cuando estamos emitiendo el valor hacia un campo destino.
        # Regla: solo cuenta si start == 0 (si start != 0, es un substr "de formato", no truncamiento de impresión).
        if ctx.vars.get("_eval_phase") == "emit" and start == 0 and len(s) > ln_int:
            mt_tag = str(ctx.vars.get("_mt_tag") or "")
            base_tpath = str(ctx.vars.get("_target_path") or "")
            src_xml = _infer_first_xml_suffix(conf.get("value", {}))
            target_path = (
                f"{base_tpath} <= {src_xml}"
                if base_tpath and src_xml
                else (base_tpath or src_xml)
            )
            if mt_tag and target_path:
                ctx.add_truncation(
                    mt_tag=mt_tag,
                    original=s,
                    max_len=ln_int,
...
def _eval_condition(dot, ctx, cond: CondSpec) -> bool:
    # Evaluación de condiciones NO debe generar truncamientos (solo el EMIT).
    prev_phase = ctx.vars.get("_eval_phase", None)
    ctx.vars["_eval_phase"] = "cond"
    try:
        op = cond.get("op")
        left = _eval_value(dot, ctx, cond.get("left", {}))
        right_val = cond.get("right", {})
        right = (
            _eval_value(dot, ctx, right_val)
        ...
        if op == "in":
            ...
            res = ls in rlist
            dbg(
                f"COND 'in' left={ls!r} list_len={len(rlist)} sample={rlist[:10]} -> {res}"
            )
            return res
        ...
        # --- helpers de string comunes en specs ---
        if op == "starts_with":
            rs = _norm_ws(right)
            res = ls.startswith(rs)
            dbg(f"COND 'starts_with' left={ls!r} right={rs!r} -> {res}")
            return res

        if op == "ends_with":
            rs = _norm_ws(right)
            res = ls.endswith(rs)
            dbg(f"COND 'ends_with' left={ls!r} right={rs!r} -> {res}")
            return res

        if op == "contains":
            rs = _norm_ws(right)
            res = rs in ls
            dbg(f"COND 'contains' left={ls!r} right={rs!r} -> {res}")
            return res
        ...
        if op == "regex":
            ...
            pat = str(right or "")
            try:
                res = re.search(pat, ls) is not None
            except re.error:
                res = False
            dbg(f"COND 'regex' left={ls!r} pat={pat!r} -> {res}")
            return res
...
def build_fields(entry_dir: str, fields_spec, global_vars=None):
    ...
    for field_tag, spec in fields_spec.items():
        ...
        if isinstance(spec, list):
            ctx = FieldContext(truncations=global_vars.setdefault("_truncations", []))
            assigned = False
            for idx, rule in enumerate(spec, start=1):
                ...
                # THEN primero
                # --- begin truncation-aware emit context (NO cambia especificaciones)
                _prev_phase = ctx.vars.get("_eval_phase", None)
                _prev_tag = ctx.vars.get("_mt_tag", None)
                _prev_tpath = ctx.vars.get("_target_path", None)
                ctx.vars["_eval_phase"] = "emit"
                ctx.vars["_mt_tag"] = field_tag
                # Para MX->MT el "path" es lógico (no XML): ayuda a ubicar dónde se truncó
                _rule_line_no = rule.get("line_no", None)
                ctx.vars["_target_path"] = (
                    f"MT/Block4/{field_tag}/line{int(_rule_line_no)}"
                    if _rule_line_no is not None
                    else f"MT/Block4/{field_tag}"
                )
                try:
                    produced = _collect_then_lines(dot, ctx, rule.get("then", {}))
                finally:
                    if _prev_phase is None:
                        ctx.vars.pop("_eval_phase", None)
                    else:
                        ctx.vars["_eval_phase"] = _prev_phase
                    if _prev_tag is None:
                        ctx.vars.pop("_mt_tag", None)
                    else:
                        ctx.vars["_mt_tag"] = _prev_tag
                    if _prev_tpath is None:
                        ctx.vars.pop("_target_path", None)
                    else:
                        ctx.vars["_target_path"] = _prev_tpath
                # --- end truncation-aware emit context
                        ...
                        "substr": {
                            "value": {
                                "concat": [
                                    {
                                        "xml_nth": {
                                            "path": ".Document.FICdtTrf.CdtTrfTxInf.InstrForNxtAgt.InstrInf",
                                            "index": 0,
                                        }
                                    },
                                    {
...
if __name__ == "__main__":
    ...
    # --- Exception Report (truncamientos detectados por substr start==0 al EMITIR) ---
    truncs = globals_ctx.get("_truncations", [])
    report = build_exception_report(truncs)

    mt_out = f"{report}\n---\n{mt}" if report else mt
