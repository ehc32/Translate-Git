...
# -----------------------------------------------------------------------
traslateId = "006"
ubicationEntry = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\006\entry"
ubicationDestiny = r"C:\Users\heoctor\Desktop\WANT\BANREP\Translete-Update\Translate Sin Comentarios\006\destiny"
...
def _fn_remove_chars(raw: str, chars: str) -> str:
    ...
    # 1. Convertimos a string y quitamos tildes (Normalización)
    # Esto convierte 'Ãº' o 'ú' en 'u'
    raw_str = str(raw)
    normalized = unicodedata.normalize("NFKD", raw_str)
    clean_text = "".join([c for c in normalized if not unicodedata.combining(c)])

    # 2. Ahora sí, eliminamos los caracteres prohibidos extra (#, /, etc.)
    if chars:
        ...
        "rules": [
            {
                ...
                "then": {"value": {"literal": _fn_now_iso8601()}},
            }
        ],
    },
    ".Document.FIToFIPmtStsRpt.GrpHdr.CreDtTm": [
        ...
        "rules": [
            {
                ...
                "then": {
                    "value": {
                        "substr": {"value": {"var": "block1"}, "start": 3, "len": 11}
                    }
                },
            }
        ],
