...
fields_spec_11S = {
    "11S": [
        # 1) msgNmId ausente -> error
        {
            "when": {
                "not": {
                    "exists": {
                        "xml": ".Document.FIToFIPmtStsRpt.TxInfAndSts.OrgnlGrpInf.OrgnlMsgNmId"
                    }
                }
            },
            "set": [
                ...
                },
                {
                    "set_var": {
                        "scope": "global",
                        "name": "ISSUE_11S",
                        "value": {"literal": "ORGNLMGNNMID_MISSING"},
                    }
                },
            ],
            "then": {"value": {"literal": ""}},
        },
        # 2) msgNmId != pacs.009.001.08 -> warning (no aplica)
        {
            "when": {
                "all": [
                    {
            ...
            "set": [
                ...
                },
                {
                    "set_var": {
                        "scope": "global",
                        "name": "ISSUE_11S",
                        "value": {"literal": "ORIGINAL_MSG_NOT_PACS00900108"},
                    }
                },
            ],
            "then": {"value": {"literal": ""}},
        },
        # 3) caso OK: msgNmId == pacs.009.001.08 y dateTime CBPR+ válido -> poblar 2 líneas
        #    Fuente de dateTime: GrpHdr/CreDtTm (pacs.002) o, si no existe, AppHdr/CreDt.
        {
            "when": {
                "all": [
                    {
                    ...
                    },
                    {
                        "any": [
                            {
                                "exists": {
                                    "xml": ".Document.FIToFIPmtStsRpt.GrpHdr.CreDtTm"
                                }
                            },
                            {"exists": {"xml": ".AppHdr.CreDt"}},
                        ]
                    },
                    {
                        "any": [
                            ...
                            {
                                "exists": {
                                    "cbpr_yymmdd": {"value": {"xml": ".AppHdr.CreDt"}}
                                }
                            },
                        ]
                    },
        ...
        # 4) msgNmId == pacs.009.001.08 pero falta dateTime -> error
        {
            "when": {
                "all": [
                    {
            ...
            "set": [
                ...
                },
                {
                    "set_var": {
                        "scope": "global",
                        "name": "ISSUE_11S",
                        "value": {"literal": "CBPR_DATETIME_MISSING"},
                    }
                },
            ],
            "then": {"value": {"literal": ""}},
        },
        # 5) msgNmId == pacs.009.001.08 y dateTime presente pero inválido -> error
        {
            "when": {
                "all": [
                    {
            ...
            "set": [
                ...
                },
                {
                    "set_var": {
                        "scope": "global",
                        "name": "ISSUE_11S",
                        "value": {"literal": "CBPR_DATETIME_INVALID"},
                    }
                },
            ],
            "then": {"value": {"literal": ""}},
...
if __name__ == "__main__":
    ...
    # :11S:
    fields_11S = build_fields(ubicationEntry, fields_spec_11S, global_vars=globals_ctx)
    lines_11S = [ln for ln in fields_11S.get("11S", []) if str(ln).strip() != ""]
    dbg("FINAL 11S lines:", lines_11S)
    if lines_11S:
        out_lines.append(f":11S:{lines_11S[0]}")
        for ln in lines_11S[1:]:
            out_lines.append(ln)
    else:
        dbg("11S NO GENERADO - revisa condiciones/validaciones")
