...
def _strip_ns(tag: str) -> str:
    """Elimina namespaces tanto de formato {uri}tag como prefix:tag"""
    if "}" in tag:
...
def _eval_value(
    ...
    # Compat: specs antiguos usan {"const": "..."} y {"expr": {...}}
    if "const" in vs:
        return str(vs["const"])

    if "expr" in vs:
        inner = vs.get("expr") or {}
        if isinstance(inner, dict):
            return _eval_value(dot, ctx, inner)
        return inner
...
def _eval_condition(dot, ctx, cond: CondSpec) -> bool:
    ...
    # NUEVO: validar BIC (8 u 11 alfanumérico)
    if op == "is_bic":
        s = str(left or "").strip().upper()
        res = (len(s) in (8, 11)) and s.isalnum()
        dbg(f"COND 'is_bic' value={s!r} -> {res}")
        return res
    ...
    if op == "len_eq":
        ...
        try:
            ...
            n = -1
        res = len(s) == n
        dbg(f"COND 'len_eq' value={s!r} n={n} -> {res}")
        return res
...
def _truncate_lines_for_field(field_tag: str, lines: list[str]) -> list[str]:
    """
    Normaliza longitudes por campo MT.

    - Para la mayoría de campos (50/51/52/53/54/55/56/57/59/70/72/77*): si una línea
      supera 35 caracteres, se parte en bloques de 35.
    - Excepciones importantes para evitar "correr" componentes:
        * 52A: máximo 2 líneas (Party Identifier opcional + BIC). Se TRUNCA a 35,
          NO se parte en múltiples líneas.
        * 52D: máximo 4 líneas (1 Party Identifier + hasta 3 Name/Address). Se TRUNCA
          a 35, NO se parte en múltiples líneas.
    """

    # 52A (opción A): 2 líneas máximo, sin split
    if field_tag == "52A":
        trimmed = []
        for ln in lines:
    ...
    # 52D (opción D): 4 líneas máximo, sin split
    if field_tag == "52D":
        trimmed = []
        for ln in lines:
            s = "" if ln is None else str(ln)
    ...
    if not field_tag.startswith(fields_35_prefixes):
        return lines
