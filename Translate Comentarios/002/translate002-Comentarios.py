...
def read_params_from_db() -> Tuple[Optional[Dict[str, List[str]]], Optional[Dict[str, List[str]]]]:
...
class FieldContext:
    ...
    @property
    def eval_phase(self) -> str:
        """Fase de evaluación: 'expr', 'emit' o 'internal'."""
        return str(self.vars.get("_eval_phase", "expr") or "expr")

    @eval_phase.setter
    def eval_phase(self, value: str) -> None:
        self.vars["_eval_phase"] = value

    @property
    def target_path(self) -> Optional[str]:
        """Ruta de destino actual usada para logs de truncamiento."""
        return self.vars.get("_target_path")

    @target_path.setter
    def target_path(self, value: Optional[str]) -> None:
        if value is None:
            self.vars.pop("_target_path", None)
        else:
            self.vars["_target_path"] = value
...
def _eval_value_mt(ctx: FieldContext, mt_fields: Dict[str, str], vs: ValueSpec) -> Any:
    ...
    if "substr" in vs:
        ...
        # Registrar truncamiento solo cuando estamos emitiendo hacia un campo destino
        if phase == "emit" and target_path and ln is not None and start == 0 and len(s) > int(ln):
    ...
    if "pad" in vs:
        ...
        if len(s) >= n:
            ...
            out = s.ljust(n, fill)
            truncated = False
        # Registrar truncamiento si hubo corte por longitud
        if truncated and phase == "emit" and target_path:
            mt_tag = _infer_first_mt_tag(vs)
            if mt_tag:
                ctx.add_truncation(
                    mt_tag=str(mt_tag),
...
def _collect_then_mt(
    ...
    """
    Evalúa el bloque "then" de una regla y devuelve los valores normalizados para
    el path destino. Marca la fase como "emit" y setea el _target_path para
    que substr/pad puedan registrar truncamientos.
    """
    out: List[str] = []
    ...
    try:
        if "value" in then_obj:
            ...
            for vs in then_obj["lines"]:
                v = _normalize_field_output(
                    target_path, _eval_value_mt(ctx, mt_fields, vs)
                )
                if isinstance(v, list):
                    out.extend([str(x) for x in v if str(x).strip() != ""])
                else:
...
def _format_truncation_exception_report(ctx: FieldContext) -> str:
    ...
    lines.append("-" * 8)
    lines.append("")
    lines.append("MT103 to PACS.008 Translation Warnings:")
    lines.append(
...
if __name__ == "__main__":
    ...
    for f in files:
        try:
            ...
            # Generar reporte de truncamientos (si los hay) y anteponerlo al XML en archivo
            ...
            if prefix:
                # Incluir también el comentario de truncamiento en la salida estándar
                print(
                    (prefix.encode("utf-8") + b'<?xml version="1.0" encoding="UTF-8"?>\n' + xml_body).decode("utf-8")
                )
            else:
                print(
